...
class OpenTTS(TTS):
    def __init__(self, lang="en-us", config=None):
        if config is None:
            self.config = Configuration.get().get("tts", {}).get("opentts", {})
        else:
            self.config = config
        ...
        self.vocoder = self.config['vocoder']
        self.denoiser_strength = self.config['denoiser_strength']
    ...
    def get_tts(self, sentence, wav_file):
        response = requests.get(self.url, params={'voice': self.voice, 'lang': self.lang, 'vocoder': self.vocoder, 'denoiser_strength': self.denoiser_strength, 'text': sentence, 'cache': "false" })
